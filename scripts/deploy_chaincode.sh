...
echo '######## - (Platform) install chaincode - ########'
setupPeerENV3
set -x
peer lifecycle chaincode package tmp/${CC_NAME}_${CC_VERSION}.tar.gz --path ${CC_PATH} --lang $CC_LANG --label ${CC_NAME}_${CC_VERSION}
peer lifecycle chaincode install tmp/${CC_NAME}_${CC_VERSION}.tar.gz
...
echo "PACKAGE_ID(Buyer):" ${PACKAGE_ID}
set -x
...
echo '######## - (Platform) approve chaincode - ########'
setupPeerENV3
set -x
# peer lifecycle chaincode queryinstalled >&tmp/log.txt
PACKAGE_ID=$(peer lifecycle chaincode queryinstalled --output json | jq -r '.installed_chaincodes[0].package_id')
...
# PACKAGE_ID=$(sed -n "/${CC_NAME}_${CC_VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" tmp/log.txt)
echo "PACKAGE_ID(Platform):" ${PACKAGE_ID}
set -x
if [[ "$CORE_PEER_TLS_ENABLED" == "true" ]]; then
    peer lifecycle chaincode approveformyorg \
    -o ${ORDERER_ADDRESS} --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA \
...
echo '######## - (Buyer) check chaincode approvals - ########'
setupPeerENV1
set -x
...
echo '######## - (Buyer) commit chaincode definition - ########'
setupPeerENV1
set -x
if [[ "$CORE_PEER_TLS_ENABLED" == "true" ]]; then
    ...
    --peerAddresses $PEER0_BUYER_ADDRESS --tlsRootCertFiles $PEER0_BUYER_TLS_ROOTCERT_FILE \
    --peerAddresses $PEER0_SELLER_ADDRESS --tlsRootCertFiles $PEER0_SELLER_TLS_ROOTCERT_FILE \
    ...
    --peerAddresses $PEER0_BUYER_ADDRESS \
    --peerAddresses $PEER0_SELLER_ADDRESS \
...
set +x
echo '######## - (Buyer) check chaincode status - ########'
setupPeerENV1
peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name ${CC_NAME} 
echo '############# END ###############'
echo '######## - (Seller) check chaincode status - ########'
setupPeerENV2
peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name ${CC_NAME} 
echo '############# END ###############'
echo '######## - (Platform) check chaincode status - ########'
setupPeerENV3
peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name ${CC_NAME} 
echo '############# END ###############'